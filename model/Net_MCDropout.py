...
class Network(nn.Module):
    ...
    def forward(self, inp):
        conv0_avg, conv1_avg, conv2_avg, conv3_avg, conv5_avg = [0 for _ in range(5)]
        for _ in range(self.mc_nums):
        ...
        conv0_avg /= self.mc_nums
        conv1_avg /= self.mc_nums
        conv2_avg /= self.mc_nums
        conv3_avg /= self.mc_nums
        conv5_avg /= self.mc_nums

        up3 = self.dec1((conv5_avg, conv3_avg))
        up2 = self.dec2((up3, conv2_avg))
        up1 = self.dec3((up2, conv1_avg))
        up0 = self.dec4((up1, conv0_avg))
